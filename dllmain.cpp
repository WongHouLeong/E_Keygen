...
void DbgPrintf(char* pszFormat, ...)//打印输出
{
	...
	strcat_s(szBuf, " [Keygen-E盾企业服务端定制版]\r\n");
...
void __declspec(naked) API_OriginalFunc(void)//修正函数
{
	VMP_BEGINU
		__asm
	{
...
void __declspec(naked) Write_OriginalFunc(void)//修正函数
{
	VMP_BEGINU
		__asm
	{
...
LONG NTAPI Handler(struct _EXCEPTION_POINTERS* ExceptionInfo)//异常函数
{
	VMP_BEGINU
		if (ExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT)
		{
			...
			{
				DWORD dwOldProtect;
				VirtualProtect((LPVOID)0x0041B9D2, 5, PAGE_EXECUTE_READWRITE, &dwOldProtect);
				*(char*)0x0041B9D2 = 0xE9;
				*(int*)0x0041B9D3 = 0xFFFE56D3;
				VirtualProtect((LPVOID)0x0041B9D2, 5, dwOldProtect, &dwOldProtect);

				VirtualProtect((LPVOID)0x006133BC, 1, PAGE_EXECUTE_READWRITE, &dwOldProtect);
				*(char*)0x006133BC = 0xCC;
				VirtualProtect((LPVOID)0x006133BC, 1, dwOldProtect, &dwOldProtect);

				VirtualProtect((LPVOID)0x006A05B8, 1, PAGE_EXECUTE_READWRITE, &dwOldProtect);
				*(char*)0x006A05B8 = 0xCC;
				VirtualProtect((LPVOID)0x006A05B8, 1, dwOldProtect, &dwOldProtect);

				VirtualProtect((LPVOID)0x00630486, 1, PAGE_EXECUTE_READWRITE, &dwOldProtect);
				*(char*)0x00630486 = 0xCC;
				VirtualProtect((LPVOID)0x00630486, 1, dwOldProtect, &dwOldProtect);

				ResumeHook();
				ExceptionInfo->ContextRecord->Eip = (DWORD)&API_OriginalFunc;
			...
			if ((DWORD)ExceptionInfo->ExceptionRecord->ExceptionAddress == 0x006A05B8)
			{
			...
			if ((DWORD)ExceptionInfo->ExceptionRecord->ExceptionAddress == 0x00630486)
			{
				DbgPrintf("ErrorKey");
				ExitProcess(-1);
				return EXCEPTION_CONTINUE_EXECUTION;
	...
	VMP_BEGINU
		if (dwReason == DLL_PROCESS_ATTACH)
		{
			...
			if (Load() && Init())
			{
				...
				{
					HMODULE hwnd = LoadLibraryA("kernel32.dll"); //装载ws2_32.dll
					if (hwnd)
					{
						HookAddr = (DWORD)GetProcAddress(hwnd, "SetCurrentDirectoryA");
						if (HookAddr)
						{
							HookAddr = HookAddr + 2;
						...
						{
							DbgPrintf("kernel32.dll Function Not Find.");
							ExitProcess(0);
						}
					}
					else
					{
						DbgPrintf("kernel32.dll Not Find.");
						ExitProcess(0);
					}
				}
			}
